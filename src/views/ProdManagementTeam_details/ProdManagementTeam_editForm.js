...
import {
  ...
  PATH_LINEITEM,
  PATH_PRODUCT,
  PATH_REPORTANDANALYZE,
  PATH_CUSTOMERPROFILE,
  PATH_PRODMANAGEMENTTEAM,
...
const EditProdManagementTeamForm = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchProdManagementTeamById = async () => {
      ...
      const jsonResp = await prodManagementTeamResponse.json();
  ...
  const submitForm = async () => {
    ...
    const resp = await makeApiCall(
      `${BASE_URL}${PATH_PRODMANAGEMENTTEAM}(${formData.EmpId})`,
      'PATCH',
            JSON.stringify({
        ...otherData,
    ...
    if (resp.ok) {
      snackbar.enqueueSnackbar('Successfully updated ProdManagementTeam', {
        variant: 'success',
      });
      navigate({ pathname: '/prodManagementTeams' });
    } else {
          ...
          <Box display="flex" justifyContent="space-between">
            ...
            <div className="action-buttons">
              ...
              <Button
                ...
                variant="contained"
                color="secondary"
                onClick={() => navigate({ pathname: '/prodManagementTeams' })}
              >
                Cancel
              </Button>
            </div>
...
export default EditProdManagementTeamForm;
